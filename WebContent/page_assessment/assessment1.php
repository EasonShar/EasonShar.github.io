...
$rs3 = mysql_query ( "select * from assessment WHERE groupAllocated='$groupID' AND groupAuthor='$groupAuthor1'" );
$array3 = mysql_fetch_array ( $rs3 );
if (isset ( $_POST ['submit'] )) {
	$mark = $_POST [mark];
	$comments = $_POST [comments];
	if (! empty ( $array3 ['mark'] ) || ! empty ( $array3 ['comments'] )) {
		echo "<script> alert('Your group has already submitted assessment.');location.href='assessment1.php';exit;</script>";
	} else {
		$submit = "update assessment set mark='$mark',comments='$comments' WHERE groupAllocated='$groupID' AND groupAuthor='$groupAuthor1'";
		mysql_query ( $submit, $link );
		echo "<script> alert('Submit successfully');location.href='assessment1.php';exit;</script>";
	}
}
?>
<!DOCTYPE html>
...
<body>
	...
	<div class="bs-example bs-example-tabs">
		<ul id="myTab" class="nav nav-tabs">
			<li class="tab-assessment-style active"><a>Group <?php echo $groupAuthor1;?> Report</a></li>
    ...
    <?php
				if (! isset ( $groupAuthor1 ) or $groupAuthor1 == "")
					die ( "error:This group has not uploaded report." );
				
				$sql = "select * from grade where groupID='$groupAuthor1'";
	...
	<div class="content">
		<div class="panel panel-default report-container">
			<div class="panel-heading">
				<h3 class="panel-title">Title: <?php echo $array2['report_title'];?></h3>
			</div>
